#!/usr/bin/env python3
"""
Script to sample the first 5 lines of CSV files (.csv, .txt) and Parquet files (.parquet) and display them on screen.
"""
...
def sample_csv_file(file_path, sample_size=5, markdown=False):
    """
    Sample the first rows of a CSV file and display on screen.
    
    Args:
        file_path (str): Path to the CSV file
        sample_size (int, optional): Number of rows to sample. Default is 5.
    ...
    try:
        ...
        # Display the data
        display_sample_data(df, file_path, start_time, markdown)
...
def sample_parquet_file(file_path, sample_size=5, markdown=False):
    """
    Sample the first rows of a Parquet file and display on screen.
...
def display_sample_data(df, file_path, start_time, markdown=False):
    """
    ...
    """
    if markdown:
        print(f"**File sampled with shape:** {df.shape}")
    ...
    if markdown:
        ...
        try:
            # Use to_markdown if available (requires tabulate)
...
def sample_all_files(directory=".", sample_size=5, markdown=False, formats=['.csv', '.txt', '.parquet']):
    """
    ...
    if markdown:
        print(f"# Found {len(files)} file(s) to sample:")
    else:
        print(f"Found {len(files)} file(s) to sample:")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Sample the first rows of CSV (.csv, .txt) and Parquet (.parquet) files and display them on screen.')
    ...
    parser.add_argument('--sample-size', '-s', type=int, default=5,
                        help='Number of rows to sample (default: 5)')
